...
returnprobs[:,0]
definitial_population(client:GeminiClient,topics:List[str],n:int,author:str)->List[str]:
    pop=[]
style_map={
...
returnpop
defmutate(client:GeminiClient,paragraph:str,prompts:List[str])->str:
    p=random.choice(prompts)
full=f"{p}\n\nParagraph:\n\"\"\"\n{paragraph}\n\"\"\"\n\nReturn only the rewritten paragraph."
returnclient.generate(full)
defcrossover(client:GeminiClient,parent1:str,parent2:str)->str:
...
returnclient.generate(prompt)
defrun_ga(target_author:str):
...
log=[]
forgeninrange(CFG["genetic_algorithm"]["generations"]):
...
ifbest>=CFG["genetic_algorithm"]["target_human_prob"]:
            print(f"Reached target >= {CFG['genetic_algorithm']['target_human_prob']}")
break
elites=[tfort,_inranked[:CFG["genetic_algorithm"]["elite_size"]]]
new_pop=list(elites)
whilelen(new_pop)<CFG["genetic_algorithm"]["population_size"]:
